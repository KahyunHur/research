---
title: "Computer-aided materials design"
---
# Computer-aided materials design

**Key Focus Areas / Related Concepts**: [[DFT|DFT]], [[Molecular Dynamics]], [[Finite Elements]], [[Artificial Intelligence]]

See also our [[Journal Publications]] for more details on our findings related to Computer-aided materials design.

![[Computer-aided materials design.jpg]]

## Overview
Imagine trying to bake the perfect cake without ever tasting the batter or opening the oven until the very end. Traditionally, discovering new materials was a slow process of "trial and error" in a physical lab. Computer-aided materials design transforms this by allowing scientists to build and test "digital twins" of materials before they are ever synthesized. By simulating how atoms interact, we can predict whether a new substance will be strong, conductive, or heat-resistant, saving years of expensive laboratory work and accelerating the development of technologies like faster microchips and cleaner energy.

Our lab specializes in bridging the gap between the microscopic world of atoms and the macroscopic world of engineering. We use [[DFT|DFT]] to calculate the quantum behavior of electrons, providing a foundation for understanding a material's core properties. From there, we employ [[Molecular Dynamics]] to simulate how thousands of atoms move and interact over time, while [[Finite Elements]] analysis allows us to predict how large-scale structures will perform under real-world physical stress. To navigate the nearly infinite combinations of the periodic table, we integrate [[Artificial Intelligence]] to "screen" millions of possibilities and identify the best candidates, turning materials discovery into a high-speed digital search.


## Related Publications
- [[2006-Macromolecules-Comparison-of-Ring-and-Linear-Polyethylene-from-Molecular|Comparison of Ring and Linear Polyethylene from Molecular Dynamics Simulations]]
- [[2011-Macromolecules-Chain-Dynamics-of-Ring-and-Linear-Polyethylene-Melts|Chain Dynamics of Ring and Linear Polyethylene Melts from Molecular Dynamics Simulations]]
- [[2010-The Journal of Chemical Physics-Mesoscopic-structure-prediction-of-nanoparticle-assembly-and-coassembly|Mesoscopic structure prediction of nanoparticle assembly and coassembly: Theoretical foundation]]
- [[2011-Angewandte Chemie International Edition-Three-Dimensionally-Isotropic-Negative-Refractive-Index-Materials-from-Block|Three-Dimensionally Isotropic Negative Refractive Index Materials from Block Copolymer Self-Assembled Chiral Gyroid Networks]]
- [[2012-Nano Letters-Predicting-Chiral-Nanostructures,-Lattices-and-Superlattices-in-Complex|Predicting Chiral Nanostructures, Lattices and Superlattices in Complex Multicomponent Nanoparticle Self-Assembly]]
- [[2017-Nature Physics-Intrinsic-photonic-wave-localization-in-a-three-dimensional-icosahedral|Intrinsic photonic wave localization in a three-dimensional icosahedral quasicrystal]]
- [[2014-Nature Communications-Linking-experiment-and-theory-for-three-dimensional-networked-binary|Linking experiment and theory for three-dimensional networked binary metal nanoparticle–triblock terpolymer superstructures]]
- [[2017-The Journal of Physical Chemistry C-Exploring-Periodic-Bicontinuous-Cubic-Network-Structures-with-Complete|Exploring Periodic Bicontinuous Cubic Network Structures with Complete Phononic Bandgaps]]
- [[2018-physica status solidi (RRL) – Rapid Research Letters-Mechanical-Metamaterials-with-Thermoresponsive-Switching-between-Positive-and|Mechanical Metamaterials with Thermoresponsive Switching between Positive and Negative Poisson's Ratios]]
